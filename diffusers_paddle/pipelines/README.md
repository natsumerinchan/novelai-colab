...
from the local path.
- [`to`](https://github.com/huggingface/diffusers/blob/5cbed8e0d157f65d3ddc2420dfd09f2df630e978/src/diffusers/pipeline_utils.py#L118) which accepts a `string` or `paddle.device` to move all models that are of type `paddle.nn.Layer` to the passed device. The behavior is fully analogous to [Paddle's `to` method](https://pytorch.org/docs/stable/generated/paddle.nn.Layer.html#paddle.nn.Layer.to).
...
**Note**: All pipelines have Paddle's autograd disabled by decorating the `__call__` method with a [`torch.no_grad`](https://pytorch.org/docs/stable/generated/torch.no_grad.html) decorator because pipelines should
...
pipe = StableDiffusionPipeline.from_pretrained("./weights")
...
pipe = StableDiffusionImg2ImgPipeline.from_pretrained("./weights")

# let's download an initial image
url = "http://bj.bcebos.com/paddlenlp/models/community/CompVis/stable-diffusion-v1-4/sketch-mountains-input.png"
...
img_url = "http://bj.bcebos.com/paddlenlp/models/community/CompVis/stable-diffusion-v1-4/overture-creations.png"
mask_url = "http://bj.bcebos.com/paddlenlp/models/community/CompVis/stable-diffusion-v1-4/overture-creations-mask.png"

init_image = download_image(img_url).resize((512, 512))
mask_image = download_image(mask_url).resize((512, 512))

pipe = StableDiffusionInpaintPipeline.from_pretrained("./weights")
