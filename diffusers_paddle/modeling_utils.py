...
WEIGHTS_NAME = "model_state.pdparams" # diffusion_paddle_model
...
def load_state_dict(checkpoint_file: Union[str, os.PathLike]):
    ...
    """
    try:
        return paddle.load(checkpoint_file, return_numpy=True)
...
class ModelMixin(nn.Layer):
    ...
    Class attributes (overridden by derived classes):
        ...
        - **config_class** ([`ConfigMixin`]) -- A subclass of [`ConfigMixin`] to use as configuration class for this
          model architecture.
        - **load_tf_weights** (`Callable`) -- A python *method* for loading a TensorFlow checkpoint in a Paddle model,
          taking as arguments:
    ...
    _automatically_saved_args = ["_diffusers_paddle_version", "_class_name", "_name_or_path"]
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        user_agent = {"file_type": "model", "framework": "paddle", "from_auto_class": from_auto_class}
        ...
        # convert dtype
        str_dtype = str(paddle_dtype)[len("paddle."):] if paddle_dtype is not None else "float32"
        # TODO 只用float32加载最后转换。
        # str_dtype = "float32"

        state_dict = {k: v.astype(str_dtype) for k, v in state_dict.items()}
    ...
    def _load_pretrained_model(
                ...
                for checkpoint_key in loaded_keys:
                    ...
                    if (
                        ...
                        mismatched_keys.append(
                            (checkpoint_key, list(state_dict[checkpoint_key].shape), model_state_dict[model_key].shape)
                        )
        ...
        if state_dict is not None:
            ...
            error_msgs = ""
            model_to_load.load_state_dict(state_dict, strict=not ignore_mismatched_sizes)
    ...
    def device(self) -> paddle.device:
        """
        `paddle.device`: The device on which the module is (assuming that all the module parameters are on the same
        device).
        """
