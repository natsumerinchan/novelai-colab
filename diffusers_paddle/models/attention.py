...
class AttentionBlock(nn.Layer):
    ...
    def __init__(
        ...
        self.group_norm = nn.GroupNorm(
            num_channels=channels, num_groups=num_groups, epsilon=eps
        )
    ...
    def forward(self, hidden_states):
        ...
        attention_probs = F.softmax(attention_scores.astype("float32"), axis=-1).astype(
...
class SpatialTransformer(nn.Layer):
    ...
    def __init__(
        self, in_channels, n_heads, d_head, depth=1, dropout=0.0, num_groups: int = 32, context_dim=None
    ):
        super().__init__()
    ...
    def forward(self, hidden_states, context=None):
        ...
        hidden_states = hidden_states.transpose([0, 2, 3, 1]).reshape([batch, height * weight, channel])
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, context=context)
        hidden_states = hidden_states.reshape([batch, height, weight, channel]).transpose([0, 3, 1, 2])
...
class CrossAttention(nn.Layer):
    ...
    def _sliced_attention(self, query, key, value, sequence_length, dim):
        ...
        hidden_states = paddle.zeros(
            (batch_size_attention, sequence_length, dim // self.heads), dtype=query.dtype
        )
        slice_size = self._slice_size if self._slice_size is not None else hidden_states.shape[0]
        for i in range(hidden_states.shape[0] // slice_size):
            start_idx = i * slice_size
            end_idx = (i + 1) * slice_size
            attn_slice = paddle.matmul(query[start_idx:end_idx], key[start_idx:end_idx], transpose_y=True) * self.scale
