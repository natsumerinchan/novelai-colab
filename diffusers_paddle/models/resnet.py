...
class Downsample2D(nn.Layer):
    ...
    def forward(self, x):
        ...
        if self.use_conv and self.padding == 0:
            pad = (0, 1, 0, 1)
            x = F.pad_new(x, pad, mode="constant", value=0)
...
class FirUpsample2D(nn.Layer):
    ...
    def _upsample_2d(self, x, weight=None, kernel=None, factor=2, gain=1):
        ...
        if self.use_conv:
            ...
            weight = weight[..., ::-1, ::-1].transpose([0, 2, 1, 3, 4])
            weight = weight.reshape((num_groups * inC, -1, convH, convW))

            x = F.conv2d_transpose(x, weight, stride=stride, output_padding=output_padding, padding=0)
...
class FirDownsample2D(nn.Layer):
    ...
    def _downsample_2d(self, x, weight=None, kernel=None, factor=2, gain=1):
        ...
        if self.use_conv:
            ...
            p = kernel.shape[0] - factor
            x = upfirdn2d_native(x, kernel, down=factor, pad=((p + 1) // 2, p // 2))
...
class ResnetBlock2D(nn.Layer):
    def __init__(
        ...
        *,
        ...
        if non_linearity == "swish":
            ...
            self.nonlinearity = Mish() # nn.Mish
    ...
    def forward(self, x, temb):
        ...
        hidden_states = self.norm1(hidden_states).astype(hidden_states.dtype)
        ...
        # make sure hidden states is in float32
        # when running in half-precision
        hidden_states = self.norm2(hidden_states).astype(hidden_states.dtype)
        hidden_states = self.nonlinearity(hidden_states)
...
def downsample_2d(x, kernel=None, factor=2, gain=1):
    ...
    kernel = kernel * gain
    p = kernel.shape[0] - factor
    return upfirdn2d_native(x, kernel, down=factor, pad=((p + 1) // 2, p // 2))
...
def upfirdn2d_native(input, kernel, up=1, down=1, pad=(0, 0)):
    ...
    out = F.pad_new(out, [0, 0, 0, up_x - 1, 0, 0, 0, up_y - 1])
    out = out.reshape([-1, in_h * up_y, in_w * up_x, minor])

    out = F.pad_new(out, [0, 0, max(pad_x0, 0), max(pad_x1, 0), max(pad_y0, 0), max(pad_y1, 0)])
