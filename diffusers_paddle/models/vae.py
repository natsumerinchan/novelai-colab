...
@dataclass
class AutoencoderKL(BaseOutput):
    """
    Output of decoding method.
...
class Encoder(nn.Layer):
    def __init__(
        ...
        # out
        self.conv_norm_out = nn.GroupNorm(num_channels=block_out_channels[-1], num_groups=norm_num_groups, epsilon=1e-6)
...
class VectorQuantizer(nn.Layer):
    ...
    def __init__(self, n_e, e_dim, beta, remap=None, unknown_index="random", sane_index_shape=False, legacy=True):
        ...
        self.embedding = nn.Embedding(self.n_e, self.e_dim, weight_attr=nn.initializer.Uniform(-1.0 / self.n_e, 1.0 / self.n_e))
...
class DiagonalGaussianDistribution(object):
    ...
    def sample(self) -> paddle.Tensor:
        ...
        sample = paddle.randn(self.mean.shape)#.to(device)
...
class AutoencoderKL(ModelMixin, ConfigMixin):
    ...
    def forward(
        self, sample: paddle.Tensor, sample_posterior: bool = False, return_dict: bool = True
    ) -> Union[AutoencoderKL, paddle.Tensor]:
        x = sample
        ...
        return AutoencoderKL(sample=dec, latent_dist=posterior)
