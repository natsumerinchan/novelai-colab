...
class DDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    Args:
        ...
        steps_offset (`int`, default `0`):
            ...
            stable diffusion.
        tensor_format (`str`): whether the scheduler expects paddle or numpy arrays.
    ...
    def __init__(
        ...
        self.final_alpha_cumprod = np.array([1.0], dtype=np.float32) if set_alpha_to_one else self.alphas_cumprod[0]
    ...
    def set_timesteps(self, num_inference_steps: int, **kwargs) -> None:
        """
    ...
    def step(
        ...
        Args:
            model_output (`paddle.Tensor` or `np.ndarray`): direct output from learned diffusion model.
            timestep (`int`): current discrete timestep in the diffusion chain.
            sample (`paddle.Tensor` or `np.ndarray`):
        ...
        if eta > 0:
            # device = model_output.device if paddle.is_tensor(model_output) else "cpu"
    ...
    def add_noise(
        self,
        original_samples: Union[paddle.Tensor, np.ndarray],
        noise: Union[paddle.Tensor, np.ndarray],
        timesteps: Union[paddle.Tensor, np.ndarray],
